...
module.exports = (server)=>{
    ...
    io.on('connection',(socket) => {
        ...
        socket.on('page_loaded',(chatroom_url)=>{
            ...
            // Promise.all([find_number_of_ppl_voted(chatroom_url),find_user_voted_which_date(user_fb_id)])
            // .then((values) =>{
            //     console.log(value);
            //     let obj=[]
            ...
            // .catch(err => console.log(err));
        //})
        find_number_of_ppl_voted(chatroom_url).then((data)=>{
                //console.log(data);
                let output=[];
                let total_number_of_ppl =10;
                for(let i=0;i<data.length;i++){
